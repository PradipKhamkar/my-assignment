...
const Header: React.FC<HeaderProps> = ({ handelSideBar }) => {
    ...
    return (
        <header className='bg-[#662671] px-5 md:py-3 py-2 fixed w-full max-w-[2500px] z-50'>
            <div className='flex justify-between items-center'>
                <div className='flex justify-center items-center gap-5'>
                    <button onClick={handelSideBar} className='lg:hidden'>
                        <MenuSvg size="30" color="white" />
                    </button>
                    <img src={require("../../assets/images/headerLogo.png")} alt="" className='md:w-52 w-44' />
                </div>
                <button onClick={() => ConfirmAlert("Confirm", "Log Out", "Are you sure you want to log out ?", handelLogOut)}>
